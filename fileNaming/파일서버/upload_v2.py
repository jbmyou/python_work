...
# 세부종류에 해당하는 것은 1에 써넣어서 세부종류 문구를 살린다.
        ...
        "기타1" : re.compile(r"집행문\s?부여|배송[가-힣]+|(채권|양도|양수|매매).*계약|화해(?!권고)|대위변제|분할|분납|상환|감면|(상속)?\s?한정\s?승인|\
            ...
            |소장|진단서|사용\s?증명원|해제\s?통지서|의견\s?청취서|느단|즈기|추심\s?요청서?|안내문?|개문|채무\s?이행\s?통지|신탁(해지|해제)?통지서?"), # 제거가 아니므로 첫 글자만 잘 찾으면 된다.
        ...
        "신용회복1" : re.compile(r"(개인)?채무\s?조정안?|.*원상\s?회복|(신청인|채무자|고객)\s?현황"),
