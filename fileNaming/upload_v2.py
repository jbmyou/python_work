...
#"1붙은 건 .*를 앞에 붙여서 아예 맨 앞으로 문서구분을 이동시켜도 괜찮겠다. 어차피 이름 중복되면 제거하니까."
        ...
        "원인서류1" : re.compile(r"(입회|가입|카드)\s?신청서|(?=녹취)|통화\s?(내용|내역)|근보증서?"),
...
def eventFnc(noKeyStem:str, testMode :bool = False)->list:
    ...
    # y-es-sn
    p_event1=re.compile("((?<=\D)|^)(?P<y>19\d\d|20"+year_com+")\s?(?P<es>준?재?[가간나느다머차카타즈본징하개회라정][가-힣]{0,2})\s?(?P<sn>[0-9]+)") # 4자리 연도
    p_event2=re.compile("((?<=\D)|^)(?P<y>[7-9][0-9]|"+year_com+")\s?(?P<es>준?재?[가간나느다머차카타즈본징하개회라정][가-힣]{0,2})\s?(?P<sn>[0-9]+)") # 2자리 연도
    # y-es
    # "\\b" 를 하거나 r"\b"를 해야함
