...
#"1붙은 건 .*를 앞에 붙여서 아예 맨 앞으로 문서구분을 이동시켜도 괜찮겠다. 어차피 이름 중복되면 제거하니까."
        "원인서류" : re.compile(r"(원인|대출)\s?서류|(입회|가입|카드)\s?신청서|(신용)?\s?대출[약신원승확상거][가-힣]*|(?<!분할)약정서|(?=녹취)|통화\s?(내용|내역)|(대출)?\s?원장|마이너스\s?대출"),
        ...
        "등본" : re.compile(r"\(?\s?법인\s?등기부?\s?(등본)?\s?\)?|\(?\s?(?<!(등기부|기부 |배당표|당표 |.결정|결정 ))등본\s?\)?|\(?\s?주민\s?등록\s?등본\s?\)?|제적\s?(등본)?") #법인등본, 제적등본 setDocu에서 따로 처리, 등본이 법원서류에서 자꾸 나오니 젤 아래로...
    }
...
def eventFnc(noKeyStem:str, testMode :bool = False)->list:
    ...
    dict = { #사건구분 검색어 / 삭제할 문서구분 키워드
        ...
        "강제집행" : [re.compile("카(?!경|기|확)|타|즈(?!기)|본|징|가|정[가|명|라|마|고|드|브|스|기]"), comp["강제집행"]], #카경: 결정,판결경정, 카기, 즈기: 기타민사신청(심판경정), 본(접수증)은 강제집행맞다.
