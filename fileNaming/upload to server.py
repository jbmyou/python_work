...
PATH_HAND = r"r'\\192.168.0.75\스캔파일\새 스캔파일(업로드)\수작업 필요"
...
def dict_refer():
    ...
    return dict(map(lambda x : (x[1].채무자키,[x[1].매각사구분, x[1].채무상태]), df_c.iterrows()))
...
for f in tqdm(file_list, total=cnt_total):
    ...
    ########## 서버 올리기 ---------------------------------------------------------
            ...
            # 관리중인 파일이면
                ...
                if depth1 == "" : #docu검사는 이미 했음. 여전히 빈칸이라면 예상치 못한 에러임
...
if cnt_total == len(log) + len(error) + cnt_hand :
    print("처리된 파일에 누수 없음")
else :
    print("처리된 파일에 누수 있음")

print(f'{cnt_total}개의 파일 중 {len(log)}개 서버 업로드, {cnt_hand}개 수작업 폴더로, {len(error)}개 예외 발생')
        # 로그 파일 나스에 저장
