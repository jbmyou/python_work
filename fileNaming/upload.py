...
p_event2=re.compile(r"(?<=\D)(\d{2})([ㄱ-ㅎ가-힣]{1,3})\s([0-9]+)|(?<=\D)(\d{2})\s([ㄱ-ㅎ가-힣]{1,3})([0-9]+)")
# 문서구분
...
p_etc = re.compile("행자부|재산\s?명시|접수증|집행문\s?부여의\s?소|자동차\s?등록\s?원부|신용\s?조회|배송[가-힣]+")
...
p_rm = re.compile(r"TAA\(회\)|SCSB|[a-zA-Z\-\d_]{2,}_[a-zA-Z\-\d_]{4,}|\(?\s?(?<!\d)\d(?!\d)\s?\)?|\(?\s?[a-zA-Z][\d]{9,}\s?\)?|\(?\s?[a-zA-Z\d]{2,}-[a-zA-Z\d]{2,}-[a-zA-Z\d]{2,}\s?\)?|\(?\s?[a-zA-Z0-9]{4,}-[a-zA-Z0-9]{4,}\s?\)?|\(?\s?\d{10,}\s?\)?|(?<![가-힣\d])[13-9]\d{5,}(?!가-힣\d])|(?<![가-힣\d])\d{3,4}(?![가-힣\d])|(?<![가-힣\d])\d{6}[\-_]\d{6}(?!가-힣\d])")
#                                 언더바로 연결된 영어,숫자 키(C1_3284792)  한자리 숫자(괄호,공백 포함)                                     2개의 하이픈으로 연결된 영어 및 숫자          하이픈 또는 언더바로 연결된 4자리 이상의 영숫자(주민번호 포함)  10자리키          날짜 아닐 6자리 이상 단독 숫자      3~4자리의 단독 숫자                 앞과 교집합있지만, 확실한 주민번호
