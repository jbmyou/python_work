...
#"1붙은 건 .*를 앞에 붙여서 아예 맨 앞으로 문서구분을 이동시켜도 괜찮겠다. 어차피 이름 중복되면 제거하니까."
        ...
        "기타1" : re.compile(r"집행문\s?부여|배송[가-힣]+|(채권|양도|양수|매매).*계약|화해(?!권고)|대위변제|분할|분납|상환|감면|(상속)?\s?한정\s?승인|\
            ...
            |배분\s?[계산|기일|내역]|[가-힣\s]*(예고|답변|준비)|이의신청|의사표시(?!용)|소송고지"), # 제거가 아니므로 첫 글자만 잘 찾으면 된다.
...
def eventFnc(noKeyStem:str, testMode :bool = False)->list:
    ...
    p_event2=re.compile(r"((?<=\D)|^)([0129]\d)\s?(준?재?[가간나느다머차카타즈본징하개회라][가-힣]{0,2})\s?([0-9]+)")
    p_event3=re.compile(r"((?<=\D)|^)(19\d\d|20[012]\d)(준?재?[가간나느다머차카타즈본징하개회라][가-힣]{0,2})\b") #반드시 띄어쓰기를 하거나 끝이나야.(& 1보다 먼저 if문에서 나오면 안됨.)
    p_event4=re.compile(r"((?<=\D)|^)([0129]\d)(준?재?[가간나느다머차카타즈본징하개회라][가-힣]{0,2})\b") #반드시 띄어쓰기를 하거나 끝이나야.(& 1보다 먼저 if문에서 나오면 안됨.)
